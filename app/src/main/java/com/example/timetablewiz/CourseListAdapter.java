...
public class CourseListAdapter extends BaseAdapter {
    ...
    public static int totalCredit = 0; // 학점
    ...
    public View getView(final int i, View view, ViewGroup viewGroup) {
        ...
        addButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                boolean validate = false; // 강의 추가가능한지 타탕성 검증용
                validate = schedule.validate(courseList.get(i).getCourseTime());
                if(!alreadyIn(courseIDList, courseList.get(i).getCourseID())) { // 이미 신청한 강의일 경우
                ...
                else if(totalCredit + courseList.get(i).getCourseCredit() > 24) { // 신청학점이 24학점을 넘을 경우 추가할 수 없도록
                    AlertDialog.Builder builder = new AlertDialog.Builder(parent.getActivity());
                    AlertDialog dialog = builder. setMessage("학점이 초과되었습니다.")
                            .setPositiveButton("다시 시도", null)
                            .create();
                    dialog.show();
                }
                    ...
                    AlertDialog.Builder builder = new AlertDialog.Builder(parent.getActivity());
                    AlertDialog dialog = builder. setMessage("중복된 시간표 입니다.")
                            .setPositiveButton("다시 시도", null)
                            .create();
                    dialog.show();
                }
    ...
    class BackgroundTask extends AsyncTask<Void, Void, String> {
        ...
        protected void onPreExecute() {
            try {
                target = "https://yeonghak97.cafe24.com/ScheduleList.php?userID=" + URLEncoder.encode(userID, "UTF-8");
            } catch (Exception e) {
                e.printStackTrace();
            }
        ...
        public void onPostExecute(String result) {
            try {
                ...
                int courseID;
                totalCredit  = 0;
                while (count< jsonArray.length()) { // 파싱
